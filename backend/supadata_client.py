...
class SupadataClient:
    ...
    def get_transcript(self, video_url: str, lang: str = "en") -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """
    ...
    async def get_transcript_async(self, video_url: str) -> Tuple[Optional[str], Optional[str], Optional[str]]:
        """
            ...
            async with httpx.AsyncClient(timeout=60.0) as client:
                try:
                    ...
                    if resp.status_code not in (200, 201, 202):
                        logger.warning(f"Supadata API error {resp.status_code}: {resp.text}")
                        return None, None, None
                    ...
                    if not content or not isinstance(content, list):
                        logger.warning("Supadata returned invalid content format (expected list of segments).")
                        return None, None, None
                    ...
                    is_cjk = detected_lang in ["zh", "zh-CN", "zh-TW", "ja", "ja-JP", "ko", "ko-KR", "chinese", "japanese", "korean"]
                    ...
                    for i, seg in enumerate(openai_segments):
                        ...
                        if not is_cjk:
                             current_parts = [p.strip() for p in current_parts] # Normalize
                    
                        joined_text = join_char.join(current_parts).strip()
                        duration = end - current_chunk["start"]
                    
                        is_punctuation = text.strip() and text.strip()[-1] in ".!?。！？"
                        ...
                        if (is_punctuation) or (is_long_duration and len(joined_text) > 20) or (is_long_text):
    ...
    async def _poll_job(self, client: httpx.AsyncClient, job_id: str, max_retries: int = 150) -> Optional[Dict]:
        """
        ...
        for i in range(max_retries):
            try:
                ...
                if status == "completed":
                    ...
                    if i % 5 == 0:
                        logger.info(f"Job {job_id} status: {status} (attempt {i}/{max_retries})")
                
                # Continue polling
            except Exception as e:
                logger.warning(f"Polling error: {e}")
        
        logger.warning(f"Supadata job {job_id} timed out after {max_retries*2} seconds.")
        return None
