...
class ContentClassification(BaseModel):
    content_form: str = Field(..., description="The form of the content, e.g. casual, tutorial, simple_explanation, deep_dive, interview, monologue, news, review, reaction, finance, narrative, marketing")
    info_structure: str = Field(..., description="The structural organization of information, e.g. thematic, sequential, argumentative, comparative, narrative_arc, problem_solution, qa_format, data_driven")
    cognitive_goal: str = Field(..., description="The primary cognitive goal for the reader, e.g. understand, decide, execute, inspire, digest, evaluate, solve, memorize")
...
class KeyPoint(BaseModel):
    ...
    evidence: str = Field(..., description="Exact quote or evidence from the text properly attributed")
...
class SummaryResponse(BaseModel):
    ...
    keypoints: List[KeyPoint] = Field(..., description="List of key points extracted from the content")
    content_type: Optional[ContentClassification] = Field(None, description="Classification metadata if available")
...
class Summarizer:
    ...
    def __init__(self):
        ...
        if self.api_key:
             ...
             logger.warning("OPENAI_API_KEY missing, Summarizer will not function correctly")
        ...
        if explicit_summary_model:
            default_summary_chain = [explicit_summary_model, explicit_fallback_model] + settings.OPENAI_SUMMARY_MODELS
        ...
        self.summary_single_max_est_tokens = self._read_int_env("OPENAI_SUMMARY_SINGLE_MAX_EST_TOKENS", 16000, min_value=3000, max_value=120000)
        self.summary_chunk_max_chars = self._read_int_env("OPENAI_SUMMARY_CHUNK_MAX_CHARS", 9000, min_value=2000, max_value=30000)
        self.summary_single_max_output_tokens = self._read_int_env("OPENAI_SUMMARY_SINGLE_MAX_OUTPUT_TOKENS", 8192, min_value=800, max_value=16384)
        self.summary_chunk_max_output_tokens = self._read_int_env("OPENAI_SUMMARY_CHUNK_MAX_OUTPUT_TOKENS", 1800, min_value=500, max_value=6000)
        self.summary_integrate_max_output_tokens = self._read_int_env("OPENAI_SUMMARY_INTEGRATE_MAX_OUTPUT_TOKENS", 2800, min_value=800, max_value=8000)
        self.summary_max_keypoints_per_chunk = self._read_int_env("OPENAI_SUMMARY_MAX_KEYPOINTS_PER_CHUNK", 10, min_value=1, max_value=24)
        self.summary_max_keypoints_final = self._read_int_env("OPENAI_SUMMARY_MAX_KEYPOINTS_FINAL", 24, min_value=6, max_value=48)
        self.summary_max_keypoints_candidates = self._read_int_env("OPENAI_SUMMARY_MAX_KEYPOINTS_CANDIDATES", 140, min_value=24, max_value=2000)
    ...
    async def _ainvoke_with_fallback(self, models: list[str], messages: list[dict], trace_config: dict = None, **kwargs):
        lc_messages = []
        for m in messages:
            if m["role"] == "system":
        ...
        for model in models:
            try:
                llm = self._get_llm(model, max_tokens=max_tokens, model_kwargs=model_kwargs)
    ...
    def _find_safe_cut_point(self, text: str) -> int:
        ...
        if last_sentence_end > 20: return last_sentence_end
        return len(text)
    ...
    def _apply_basic_formatting(self, text: str) -> str:
        ...
        for i, part in enumerate(parts):
            cur += part
            if i % 2 != 0:
                if cur.strip(): sentences.append(cur.strip())
                cur = ""
        if cur.strip(): sentences.append(cur.strip())
        ...
        for s in sentences:
            ...
            sentence_count += 1
            should_break = False
            if len(candidate) > 400 and cur: should_break = True
            elif len(candidate) > 200 and sentence_count >= 3: should_break = True
            elif sentence_count >= 6: should_break = True
        ...
        for i, c in enumerate(final_chunks):
            ...
            try:
                ...
                if trace_config:
                    ...
                    chunk_config["metadata"] = {**chunk_config.get("metadata", {}), "chunk_index": i}
        ...
        try:
            ...
            for para in paragraphs:
                ...
                if current_tokens + para_tokens > max_chunk_tokens and current_chunk:
                    chunk_text = '\n\n'.join(current_chunk)
                    organized_chunks.append(await self._organize_single_chunk(chunk_text, lang_instruction))
                    current_chunk = [para]
    ...
    def _split_long_paragraph(self, paragraph: str) -> list:
        ...
        for s in sentences:
            ...
            if cur_len + slen > 200 and cur_para:
                ...
                cur_para.append(s)
                cur_len += slen
        if cur_para: split.append(' '.join(cur_para))
        return split
    ...
    async def summarize(
        ...
        try:
            ...
            if estimated_tokens <= max_summarize_tokens:
                ...
                logger.info(f"Text long ({estimated_tokens} tokens), using chunked summary")
    ...
    async def classify_content(self, transcript: str, trace_metadata: dict = None) -> dict:
        transcript_sample = transcript[:15000] # Increased limit since we trust structural output
        ...
        try:
            ...
            lc_config = {
                "run_name": trace_config.get("name", "Content Classification") if trace_config else "Content Classification",
                "metadata": trace_config.get("metadata", {}) if trace_config else {},
            }

            classification: ContentClassification = await structured_llm.ainvoke(messages, config=lc_config)
            ...
            if result["content_form"] not in valid_forms: result["content_form"] = "casual"
            if result["info_structure"] not in valid_structures: result["info_structure"] = "thematic"
            if result["cognitive_goal"] not in valid_goals: result["cognitive_goal"] = "digest"
            ...
            return {"content_form": "casual", "info_structure": "thematic", "cognitive_goal": "digest", "confidence": 0.0}

    def _build_v2_dynamic_prompt(self, classification: dict, language_name: str, target_language: str) -> tuple[str, str]:
    ...
    async def _summarize_v2_classified(self, transcript: str, target_language: str, existing_classification: dict | None = None, trace_metadata: dict = None) -> str:
        ...
        user_prompt = SUMMARY_V2_USER_TEMPLATE.format(language_name=language_name, transcript=transcript, content_type_info=content_type_info)
        ...
        try:
            ...
            for model in self.summary_models:
                try:
                    llm = self._get_llm(model, max_tokens=self.summary_single_max_output_tokens)
                    ...
                    lc_config = {
                        "run_name": trace_config.get("name", "Summary Generation (V2)") if trace_config else None,
                        "metadata": trace_config.get("metadata", {}) if trace_config else {},
                        **{k: v for k, v in (trace_config or {}).items() if k not in ["name", "metadata"]}
                    }

                    summary_obj: SummaryResponse = await structured_llm.ainvoke(messages, config=lc_config)
    ...
    def _normalize_lang_code(self, lang: Optional[str]) -> str:
        ...
        name_map = {
            "chinese": "zh", "japanese": "ja", "korean": "ko", "english": "en",
    ...
    def _parse_script_raw_payload(self, script_raw_json: Optional[str]) -> tuple[str, list[dict]]:
        if not script_raw_json: return "unknown", []
        try:
        ...
        if not isinstance(segs, list): return lang, []
        out = []
        for s in segs:
    ...
    def _build_keypoint_query(kp: dict) -> str:
        ...
        detail = str(kp.get("detail", "")).strip()[:100] # Reduced from 220 to reduce noise
    ...
    def _tokenize_for_match(self, text: str, lang: str = "en") -> set[str]:
        ...
        if self._is_cjk_text(s):
            compact = s.replace(" ", "")
            if len(compact) <= 1: return {compact} if compact else set()
    ...
    def _score_segment_match(self, *, query: str, query_tokens: set[str], seg_text: str, seg_tokens: set[str]) -> float:
        if not seg_text: return 0.0
        qn, sn = self._normalize_for_match(query), self._normalize_for_match(seg_text)
        if not qn or not sn: return 0.0
        score = 0.0
        if len(qn) >= 8 and qn in sn: score += 30.0
    ...
    def _inject_keypoint_timestamps(self, summary_obj: dict, segments: list[dict], lang: str = "en") -> dict:
        if not isinstance(summary_obj, dict): return summary_obj
        kps = summary_obj.get("keypoints", [])
        if not isinstance(kps, list) or not segments: return summary_obj
        
        seg_cache = []
        for s in segments:
            seg_cache.append((s, self._tokenize_for_match(s.get("text", ""), lang=lang)))

        for kp in kps:
            if not isinstance(kp, dict): continue
            if isinstance(kp.get("startSeconds"), (int, float)) and kp.get("startSeconds") is not None: continue
            ...
            for i, (seg, seg_tokens) in enumerate(seg_cache):
                scores.append(self._score_segment_match(query=query, query_tokens=q_tokens, seg_text=seg.get("text", ""), seg_tokens=seg_tokens))
            
            if not scores: continue
            best_idx, best_score = -1, -1e9
            for i, sc in enumerate(scores):
                if sc > best_score: best_score, best_idx = sc, i
            
            if best_idx == -1 or best_score < self.summary_match_threshold: continue

            start_idx, end_idx = best_idx, best_idx
            expansion_ratio = 0.6
            min_expansion_score = self.summary_match_threshold * 0.8
            
            while start_idx > 0 and (scores[start_idx-1] >= best_score*expansion_ratio or scores[start_idx-1] > min_expansion_score):
                 start_idx -= 1
            while end_idx < len(segments)-1 and (scores[end_idx+1] >= best_score*expansion_ratio or scores[end_idx+1] > min_expansion_score):
                 end_idx += 1
        ...
        try: summary_obj["version"] = max(int(summary_obj.get("version", 1) or 1), 2)
        except: summary_obj["version"] = 2
        return summary_obj

    async def summarize_in_language_with_anchors(self, transcript: str, *, summary_language: str, video_title: str | None = None, script_raw_json: str | None = None, existing_classification: dict | None = None, trace_metadata: dict = None) -> str:
        base = await self.summarize(transcript, summary_language, video_title, existing_classification=existing_classification, trace_metadata=trace_metadata)
        raw_info_lang, segments = self._parse_script_raw_payload(script_raw_json)
        if not segments: return base
        try:
            obj = json.loads(base)
            if isinstance(obj, dict):
                lang_code = self._normalize_lang_code(raw_info_lang).split("-")[0] if raw_info_lang != "unknown" else "en"
    ...
    def _validate_summary_json_v1(self, obj: dict, target_language: str) -> dict:
        if not isinstance(obj, dict): raise ValueError("Summary JSON must be an object")
        ...
        if isinstance(keypoints, list):
            for kp in keypoints:
                ...
                if not title and not detail: continue
                out = {"title": title or detail[:48], "detail": detail, "evidence": str(kp.get("evidence") or "").strip()}
                if "startSeconds" in kp: out["startSeconds"] = float(kp["startSeconds"])
                if "endSeconds" in kp: out["endSeconds"] = float(kp["endSeconds"])
                normalized_kps.append(out)
        return {"version": version, "language": language, "overview": overview, "keypoints": normalized_kps}

    async def translate_summary_json(self, summary_json: str, *, target_language: str, trace_metadata: dict = None) -> str:
        if not summary_json: return summary_json
        try: src_obj = json.loads(summary_json)
        except: return summary_json
        if not isinstance(src_obj, dict): return summary_json
        src_obj = self._validate_summary_json_v1(src_obj, str(src_obj.get("language") or "unknown"))
        if not self.api_key: return json.dumps(src_obj, ensure_ascii=False)
        ...
        user_prompt = json.dumps({"targetLanguage": tgt, "input": src_obj}, ensure_ascii=False)
        
        trace_config = None
        if trace_metadata:
             trace_config = {
                 "name": "Translate Summary",
                 "metadata": {**trace_metadata.get("metadata", {}), "target_language": target_language},
                 **{k: v for k, v in trace_metadata.items() if k != "metadata"}
             }
        ...
        try:
            response = await self._ainvoke_with_fallback(
                ...
                trace_config=trace_config,
                response_format={"type": "json_object"} if self.use_response_format_json else None
            )
    ...
    async def _summarize_with_chunks_json(self, transcript: str, target_language: str, max_summarize_tokens: int, trace_metadata: dict = None) -> str:
        ...
        for i, chunk in enumerate(chunks):
            system_prompt = SUMMARY_CHUNK_SYSTEM.format(language_name=language_name, target_language=target_language)
            user_prompt = SUMMARY_CHUNK_USER.format(language_name=language_name, chunk_text=chunk)
            
            trace_config = None
            if trace_metadata:
                 trace_config = {
                     "name": "Summary Chunk",
                     "metadata": {**trace_metadata.get("metadata", {}), "chunk_index": i},
                     **{k: v for k, v in trace_metadata.items() if k != "metadata"}
                 }
        ...
        return await self._integrate_chunk_summaries(valid_summaries, target_language, trace_metadata=trace_metadata)

    async def _integrate_chunk_summaries(self, chunk_summaries: list[str], target_language: str, trace_metadata: dict = None) -> str:
        combined = "\n\n".join(chunk_summaries)
        language_name = self.language_map.get(target_language, "English")
        
        system_prompt = SUMMARY_INTEGRATE_SYSTEM.format(language_name=language_name, target_language=target_language)
        user_prompt = SUMMARY_INTEGRATE_USER.format(language_name=language_name, combined_summaries=combined)
