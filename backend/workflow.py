...
async def check_cache(state: VideoProcessingState) -> Dict:
    ...
    try:
        ...
        existing_task = db_client.find_latest_completed_task_by_url(normalized_url)
        if not existing_task:
            # Fallback: try original URL in case DB has old non-normalized entries
            existing_task = db_client.find_latest_completed_task_by_url(
                state["video_url"]
            )

        if existing_task:
            logger.info(
                f"Cache hit: {state['video_url']} found in task {existing_task['id']}"
            )
...
async def fetch_data(state: VideoProcessingState) -> Dict:
    ...
    if state["is_youtube"]:
        try:
            db_client.update_task_status(
                state["task_id"], status="processing", progress=20
            )
            md, raw, lang = await supadata_client.get_transcript_async(
                state["video_url"]
            )
            if md and raw:
                # Store
                ...
                try:
                    ...
                    script_raw_out = next(
                        (o for o in outputs if o["kind"] == "script_raw"), None
                    )
                    if script_raw_out:
                    ...
                    script_out = next(
                        (o for o in outputs if o["kind"] == "script"), None
                    )
                    if script_out:
                        db_client.update_output_status(
                            script_out["id"],
    ...
    if transcript_result:
        try:
            ...
            # Extract info
            updates["video_title"] = (
                info.get("title") or state.get("video_title") or "Unknown"
            )
            updates["thumbnail_url"] = info.get("thumbnail") or state.get(
                "thumbnail_url"
            )
        ...
        try:
            ...
            (
                ...
                direct_audio_url,
                info,
            ) = await video_processor.download_and_convert(state["video_url"], TEMP_DIR)
        ...
        if updates.get("direct_audio_url"):
            ...
            if "audio" not in existing_kinds:
                db_client.create_task_output(
                    state["task_id"], state["user_id"], kind="audio"
                )
                current_outputs = db_client.get_task_outputs(
                    state["task_id"]
                )  # refresh
...
async def transcribe(state: VideoProcessingState) -> Dict:
    ...
    if not script_text and state.get("audio_path"):
        ...
        if script_out:
            db_client.update_output_status(
                script_out["id"], status="processing", progress=10
            )
        ...
        try:
            (
                ...
                raw_json,
                detected_language,
            ) = await transcriber.transcribe_with_raw(state["audio_path"])
            script_raw = raw_json
    ...
    if script_text:
        ...
        if not is_already_clean:
            ...
            try:
                ...
                # Choose optim method
                if state["is_youtube"] and state.get(
                    "transcript_raw"
                ):  # Supadata source usually
                    # Use fast regex clean
                    optimized = summarizer.fast_clean_transcript(script_text)
                else:
                    # LLM clean
                    optimized = await summarizer.optimize_transcript(
                        script_text, trace_metadata=trace_meta
                    )
...
async def classify(state: VideoProcessingState) -> Dict:
    ...
    if settings.SUMMARY_STRATEGY == "v2_classified" and not state.get(
        ...
        if class_out:
            db_client.update_output_status(
                class_out["id"], status="processing", progress=10
            )
            try:
                trace_meta = {
                    "session_id": str(state["task_id"]),
                ...
                classification_result = await summarizer.classify_content(
                    state["transcript_text"], trace_metadata=trace_meta
                )
                ...
                db_client.update_output_status(
                    class_out["id"], status="error", error=str(e)
                )
...
async def summarize(state: VideoProcessingState) -> Dict:
    ...
    summary_source_out = next(
        (o for o in outputs if o["kind"] == "summary_source"), None
    )
    ...
    try:
        ...
        if not source_json:
            if summary_source_out:
                db_client.update_output_status(
                    summary_source_out["id"], status="processing", progress=20
                )
        ...
        if summary_out and summary_out.get("status") != "completed":
            db_client.update_output_status(
                summary_out["id"], status="processing", progress=35
            )
            ...
            if str(requested_lang).lower() != str(transcript_lang).lower():
                final_json = await summarizer.translate_summary_json(
                    source_json, target_language=requested_lang
                )

            updates["final_summary_json"] = final_json
            db_client.update_output_status(
                summary_out["id"], status="completed", progress=100, content=final_json
            )

        # 3. Comprehension Brief (Learning Tab)
        brief_out = next(
            (o for o in outputs if o["kind"] == "comprehension_brief"), None
        )
        if brief_out and brief_out.get("status") != "completed":
            db_client.update_output_status(
                brief_out["id"], status="processing", progress=10
            )
            ...
            db_client.update_output_status(
                brief_out["id"], status="completed", progress=100, content=brief_json
            )
        ...
        for o in outputs:
            if (
                o["kind"] in ("summary", "summary_source", "comprehension_brief")
...
def build_graph():
    ...
    workflow.add_conditional_edges(
        "check_cache",
        route_after_cache,
        {"cleanup": "cleanup", "fetch_data": "fetch_data", "summarize": "summarize"},
    )
