...
import { ArrowLeft, CheckCircle2, Clock, FileText, Languages, PlayCircle, Subtitles, RotateCcw } from "lucide-react"
                    ...
                    {(task.status === "processing" || task.status === "pending") && (
                        <div className="space-y-4 mb-8 p-6 bg-primary/5 rounded-xl border border-primary/10">
                            <div className="flex items-center gap-3">
                                ...
                                <span className="font-medium text-primary">Processing video...</span>
                            ...
                            <p className="text-xs text-muted-foreground text-center">
                                ...
                                <br />We are transcribing, summarizing, and translating simultaneously.
                            </p>
                        </div>
                    )}
                    ...
                    <Tabs defaultValue="script" value={activeTab} onValueChange={setActiveTab} className="w-full">
                        <TabsList className="grid w-full grid-cols-3 bg-secondary/50 p-1">
                            <TabsTrigger value="script" className="gap-2 data-[state=active]:bg-primary data-[state=active]:text-black font-medium">
                                <Subtitles className="h-4 w-4" /> Full Script
                            </TabsTrigger>
                        ...
                        <TabsContent value="summary" className="mt-6 space-y-4">
                            <OutputCard output={summary} onRetry={handleRetry} placeholder="Summary will appear here..." />
                        </TabsContent>

                        <TabsContent value="translation" className="mt-6 space-y-4">
                            {translations.length === 0 && <div className="text-center p-8 text-muted-foreground bg-black/20 rounded-xl">No translations requested.</div>}
                            {translations.map(t => (
                                <div key={t.id}>
                                    <h3 className="mb-3 font-semibold text-primary/80 flex items-center gap-2">
                                        <Languages className="h-4 w-4" />
                                        {t.locale?.toUpperCase()} Translation
                                    </h3>
                                    <OutputCard output={t} onRetry={handleRetry} />
                                </div>
                            ))}
                        </TabsContent>
                    </Tabs>
        ...
        return (
            <Card className="bg-red-950/20 border-red-500/30">
                <CardContent className="p-6 flex flex-col items-center gap-4 text-red-300">
                    <p>Failed to generate content: {output.error_message}</p>
                    <Button variant="outline" size="sm" onClick={() => onRetry(output.id)} className="gap-2">
                        <RotateCcw className="h-4 w-4" /> Retry Segment
                    </Button>
                </CardContent>
            </Card>
        )
            ...
            <Card className="bg-black/20 border-white/5 min-h-[300px] flex items-center justify-center">
                <CardContent className="p-10 text-center space-y-4">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-muted-foreground mt-1">{output.progress}% complete</p>
                    </div>
                </CardContent>
            </Card>
        )
                ...
                <div className="prose prose-invert prose-lg max-w-none prose-headings:text-primary prose-a:text-primary prose-strong:text-white prose-strong:font-bold">
                    <ReactMarkdown
                        components={{
                            ...
                            strong: ({ node, ...props }) => <strong className="text-primary font-mono bg-primary/10 px-1 rounded" {...props} />
                        }}
                    >
                        {output.content}
                    </ReactMarkdown>
