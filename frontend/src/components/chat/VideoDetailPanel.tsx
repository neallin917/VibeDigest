...
import { X, Lightbulb, CheckCircle, StickyNote, PlayCircle } from 'lucide-react'
...
export function VideoDetailPanel({ 
  ...
  useEffect(() => {
    ...
    const channel = supabase.channel(`task_ctx_${taskId}`)
      ...
      }, (payload: any) => setTask(payload.new))
      .on('postgres_changes', {
      ...
      <div className="flex-1 overflow-y-auto space-y-5 px-1 pb-4 custom-scrollbar">
        ...
        {!summary && task.status === 'processing' && (
            <div className="p-8 text-center">
                <div className="inline-block animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-500 dark:border-emerald-500 mb-2"></div>
        ...
        {summary?.keypoints?.map((kp, idx) => (
            <div 
                ...
                className={cn(
                    ...
                    // Dark: Dark Glass
                    "dark:bg-[#1A1A1A] dark:border-white/10 dark:shadow-none dark:hover:bg-[#222]"
                )}
            >
            <div className="flex justify-between items-start mb-3">
                <span className="bg-indigo-100/90 text-indigo-700 dark:bg-emerald-500/20 dark:text-emerald-300 text-[10px] font-bold px-2.5 py-1 rounded-full border border-indigo-200 dark:border-emerald-500/30 uppercase tracking-wider flex items-center gap-1.5 shadow-sm">
                <Lightbulb className="w-3.5 h-3.5" />
                Insight {idx + 1}
                </span>
                {kp.startSeconds !== undefined && (
                    <span className="text-[10px] font-mono text-slate-500 dark:text-slate-400 bg-white/50 dark:bg-white/10 px-2 py-1 rounded-md backdrop-blur-sm flex items-center gap-1 hover:bg-white/80 transition-colors">
