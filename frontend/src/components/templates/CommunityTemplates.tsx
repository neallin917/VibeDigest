...
export function CommunityTemplates({ limit, showHeader = true, initialTasks = [] }: { limit?: number, showHeader?: boolean, initialTasks?: Task[] }) {
    ...
    useEffect(() => {
        // If we have initial tasks, we don't need to fetch on mount unless limit changes and invalidates it?
        // Actually, if initialTasks are provided, we should prefer them. 
        // We only fetch if we DON'T have initial tasks or if we want to refresh.
        // For now, let's respect the original logic but fix the linter error.
