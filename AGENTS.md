...
_Note: All Python commands MUST be run with `uv`._
...
3.  **Graph Execution (Stateful)**:
    - **Node: `check_cache`**: Checks DB for existing results using URL/Params.
      - _Hit_: Clones results to new task and **finishes**.
      - _Miss_: Continues to `fetch_data`.
    - **Node: `fetch_data`**:
...
> **Constraint**: Automated CI and Unit tests must be **cost-free**. Local Integration/E2E tests MAY use real APIs.

### 10.1 Token Consumption Policy

- **Unit/CI Tests**: **NEVER** call real paid APIs (OpenAI, DeepSeek, Anthropic). Use mocks.
- **Local Integration/E2E**: Real API usage IS PERMITTED locally for verifying quality and end-to-end flows.
- **Billing**: **NEVER** perform real billing transactions (Use Sandbox).
- **Downloads**: Real video downloads are permitted in local E2E tests but discouraged in CI due to bandwidth/time.
- **Mocking**: For standard suites, use `unittest.mock` to intercept `transcriber.transcribe`, `summarizer.summarize`, and `video_processor.download`.
